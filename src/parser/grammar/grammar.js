...
var grammar = {
    Lexer: lexer,
    ParserRules: [
    {"name": "main", "symbols": ["statementBlock"], "postprocess": data => data[0]},
    ...
    {"name": "minecraftComment", "symbols": [{"literal":"#"}, "_", (lexer.has("comment") ? {type: "comment"} : comment)], "postprocess": data => ({type: 'comment', comment: data.join('')})},
    ...
    {"name": "commandValue_", "symbols": [(lexer.has("expression") ? {type: "expression"} : expression)], "postprocess": data => ({type: 'expression', data: data[0].value})},
    {"name": "_", "symbols": []},
